...
test(
  ...
  async (t: TestController) => {
    ...
    await h(t).withLog(
      'All conversations should not be hidden before login',
      async () => {
    ...
    await h(t).withLog(
      'And I set user skip_close_conversation_confirmation is true before login',
      async () => {
        await user.sdk.glip.updateProfile(user.rcId, {
    ...
    await h(t).withLog(
      'Then I can find the 3 conversations in conversation list',
      async () => {
    ...
    for (const key in groupList) {
      ...
      await h(t).withLog(
        `When I open a ${key} conversation and then click close conversation button`,
        async () => {
          await item.enter();
      ...
      await h(t).withLog(
        `Then the ${key} conversation should be remove from conversation list.`,
        async () => {
          await t
            .expect(
              directMessagesSection.conversationByIdEntry(currentGroupId).exists,
            )
            .notOk();
        },
      );
      ...
      await h(t).withLog(
        'And Content panel should navigate to Blank page',
        async () => {
          await t.wait(2e3);
          ...
          await t
            .expect(
              app.homePage.messagePanel.conversationPage.messageInputArea.exists,
            )
            .notOk();
        },
      );
...

    {
      ...
      const key = 'favorite';
      await h(t).withLog(
        `When I click more Icon of a ${key} conversation with UMI`,
        async () => {
      ...
      await h(t).withLog(
        'Then the close button should not be show',
        async () => {
...
test(
  ...
  async (t: TestController) => {
    ...
    await h(t).withLog(
      'When I open conversation B and close conversation A',
      async () => {
        await pvtChat.enter();
    ...
    await h(t).withLog(
      'Then  conversation A should be remove from conversation list.',
      async () => {
        await t.wait(2e3);
        await t
          .expect(dmSection.conversationByIdEntry(currentGroupId).exists)
          .notOk();
      },
    );
...
test(
  ...
  async (t: TestController) => {
    ...
    await h(t).withLog(
      'And I set user skip_close_conversation_confirmation is False before login',
      async () => {
        await user.sdk.glip.updateProfile(user.rcId, {
    ...
    await h(t).withLog(
      "When I click conversation A's close buttom",
      async () => {
    ...
    await h(t).withLog('Then a confirm dialog should be popup', async () => {
      ...
      await t
        .expect(
          dialog.dontAskAgainCheckbox
            .parent('label')
            .find('span')
            .withText(checkboxLabel).exists,
        )
        .ok();
      await t
    ...
    await h(t).withLog(
      `When I don't select "Don't ask me again" then click "Close Conversation" button`,
      async () => {
    ...
    await h(t).withLog(
      'The popup dialog dissmis and conversation A is unvisible',
      async () => {
    ...
    await h(t).withLog(
      "When I click conversation B's close buttom",
      async () => {
        await team.openMoreMenu();
        await app.homePage.messagePanel.moreMenu.close.enter();
      },
    ...
    await h(t).withLog(
      'Then should be show the confirm dialog again',
      async () => {
        await t.expect(dialog.getSelector('h2').withText(title).exists).ok();
        await t.expect(dialog.getSelector('p').withText(content)).ok();
        await t
...
test(
  ...
  async (t: TestController) => {
    ...
    await h(t).withLog(
      `When I select "Don't ask me again" then click "Close Conversation" button`,
      async () => {
    ...
    await h(t).withLog(
      'The popup dialog disappear and conversation A should be closed',
      async () => {
        await t.expect(dialog.exists).notOk();
        await t.expect(pvtChat.exists).notOk();
      },
    ...
    await h(t).withLog(
      'Then should not show  the confirm dialog agin',
      async () => {
        await t.expect(dialog.exists).notOk();
      },
    );
...
test(
  ...
  async (t: TestController) => {
    ...
    await h(t).withLog(
      'Given I have an extension with 2 private chat, 2 team chat, and 1 group team',
      async () => {
        pvtChatId = (await user.sdk.platform.createGroup({
          type: 'PrivateChat',
          members: [user.rcId, users[5].rcId],
    ...
    await h(t).withLog(
      'And other user send post to each conversation',
      async () => {
        await teamsSection.expand();
    ...
    await h(t).withLog(
      ...
      async () => {
        ...
        await directMessagesSection.expand();
        directMessageItem = directMessagesSection.conversationByIdEntry(
          pvtChatId,
        );
