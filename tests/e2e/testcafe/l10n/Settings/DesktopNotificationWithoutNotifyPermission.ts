...
fixture('Settings/DesktopNotificationWithoutNotifyPermission')
  .beforeEach(setupCase(BrandTire.RCOFFICE))
  .afterEach(teardownCase());

test(formalName('Check Phone Settings', ['P2', 'Settings', 'DesktopNotificationWithoutNotifyPermission', 'V1.6', 'Knight.Shen']), async (t) => {
  ...
  await h(t).withLog('Then I can see Off item in select list', async () => {
    await t.expect(notificationAndSoundsSettingPage.DialogOKButton.exists).ok();
  })

  await h(t).log('And I capture screenshot', { screenshotPath: 'Jupiter_Settings_NotificationNoAllowed' })
})
