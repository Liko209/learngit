...
class MentionsSettingHandler extends AbstractSettingEntityHandler<
  NOTIFICATION_OPTIONS
> {
  ...
  async updateValue(value: NOTIFICATION_OPTIONS) {
    await this._profileService.updateSettingOptions([
      { value, key: SETTING_KEYS.EMAIL_MENTION },
    ]);
  }
  async fetchUserSettingEntity() {
    ...
    const settingItem: UserSettingEntity<NOTIFICATION_OPTIONS> = {
      ...
      valueType: 1,
      parentModelId: 1,
      value: profile[SETTING_KEYS.EMAIL_MENTION],
      id: SettingEntityIds.Notification_Mentions,
  ...
  async onProfileEntityUpdate(
    ...
    if (
      profile[SETTING_KEYS.EMAIL_MENTION] !== this.userSettingEntityCache.value
    ) {
      this.notifyUserSettingEntityUpdate(await this.getUserSettingEntity());
    }
  }
}
export { MentionsSettingHandler };
