...
fixture('Call/ActiveCallPark')
  .beforeEach(setupCase(BrandTire.RCOFFICE))
  .afterEach(teardownCase());

test(formalName('Check Park for an active call', ['P2', 'Call', 'ActiveCallPark', 'V1.6', 'Jenny.Cai']), async (t) => {
  ...
  if (SITE_ENV == 'XMN-UP') {
    ...
    otherUser.company.number = '(877) 226-2046';
  ...
  await h(t).withLog('And I click Record > Stop > Click Call Actions > Park', async () => {
    await t.wait(3000);
  ...
  await h(t).log('Then I take screenshot', { screenshotPath: 'Jupiter_Call_ActiveCallParkFailed'})

  await h(t).withLog('When I wait 3s and park again', async () => {
    await t.wait(3000);
    await telephonyDialog.clickActionsButton();
    await telephonyDialog.clickParkActionButton();
  })

  await h(t).log('Then I capture screenshot', { screenshotPath: 'Jupiter_Call_ActiveCallParkSuccessfully'})
