...
function browserBuild(
    ...
    dtsGenerator.default({
      ...
      resolveModuleId: params => {
        ...
        if (currentModuleId.indexOf("/index") !== -1) {
          return `${pkgName}/${currentModuleId.slice(
            0,
            currentModuleId.indexOf("/index")
          )}`;
        }
      },
      resolveModuleImport: params => {
        ...
        if (index !== -1) {
          const resolved = `${pkgName}/${filenameToMid(
            path.join(
              path.dirname(currentModuleId),
              importedModuleId.slice(0, index)
            )
          )}`;
          return resolved;
        }
  ...
  if (multipleEntry) {
    return new Promise(resolve => {
      execSync(`tsc -p ${p} --outDir ${destination} --allowJs false -d`, {
        stdio: [0, 1, 2]
      });
      execSync(`node ./scripts/create-package-file.js ${p} ${destination}`, {
        stdio: [0, 1, 2]
      });
      resolve();
    });
