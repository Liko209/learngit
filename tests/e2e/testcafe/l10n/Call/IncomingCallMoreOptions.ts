...
fixture('Call/IncomingCallMoreOptions')
  .beforeEach(setupCase(BrandTire.RCOFFICE))
  .afterEach(teardownCase());

test(formalName('Check More options in an incoming call', ['P2', 'Call', 'IncomingCallMoreOptions', 'V1.4', 'Jenny.Cai']), async (t) => {
  ...
  await h(t).log('Then I take screenshot', { screenshotPath: 'Jupiter_Call_CallOptions'})
  ...
  await h(t).log('And I capture screenshot', { screenshotPath: 'Jupiter_Call_CallOptionsReply'})
  ...
  await h(t).log('And I capture screenshot', { screenshotPath: 'Jupiter_Call_ReplyOptions'})

  await h(t).withLog('When I click Will call you back in', async () => {
    await telephonyDialog.clickReplyWithWillCallBackEntryButton();
  })

  await h(t).log('Then I capture screenshot', { screenshotPath: 'Jupiter_Call_ReplyWithWillCallBack'})
  ...
  await h(t).log('And I capture screenshot', { screenshotPath: 'Jupiter_Call_ReplyWithInMeeting'})
});

test(formalName('Check Call options in an incoming call', ['P2', 'Call', 'IncomingCallMoreOptions', 'V1.6', 'Jenny.Cai']), async (t) => {
  const loginUser = h(t).rcData.mainCompany.users[4];
  const otherUser = h(t).rcData.mainCompany.users[5];
  const app = new AppRoot(t);
  ...
  const telephonyDialog = app.homePage.telephonyDialog;
  await h(t).withLog('When I receive incoming call and hover Call option button', async () => {
    await telephonyDialog.ensureLoaded();
    await telephonyDialog.hoverMoreOptionsButton();
  })
  await h(t).log('Then I take screenshot', { screenshotPath: 'Jupiter_Call_CallOptions'})
