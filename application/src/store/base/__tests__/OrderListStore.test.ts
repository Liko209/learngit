...
describe('OrderListStore', () => {
  let listStore: OrderListStore;
  let disposer: Function;

  const preInsertItems = [{ id: 1, sortKey: 1 }, { id: 2, sortKey: 2 }];

  beforeEach(() => {
    listStore = new OrderListStore();
    listStore.upsert(preInsertItems);

    disposer = autorun(() => {
      const itemStr = JSON.stringify(listStore.getItems());
      console.log(`========> ${itemStr}`);
    });
  });
  ...
  it('upsert', () => {
    ...
    listStore.upsert([{ id: 3, sortKey: 1.5 }]);
    ...
    listStore.upsert([{ id: 3, sortKey: 3 }]);
    checkListStore(listStore, [
      { id: 1, sortKey: 1 },
      { id: 2, sortKey: 2 },
      { id: 3, sortKey: 3 },
  ...
  it('removeById', () => {
    ...
    checkListStore(listStore, [{ id: 2, sortKey: 2 }]);
  });
  ...
  it('getById', () => {
    expect(listStore.getById(1)).toEqual({ id: 1, sortKey: 1 });
  });
});
