...
describe('Read', () => {
  ...
  class init {
    @test(
      'should render correct when button is icon type under read status [JPT-2461]',
    )
    @mockEntity({
    ...
    t1() {
      const wrapper = mountWithTheme(
        <Read id={1234} type={BUTTON_TYPE.ICON} />,
      );
    ...
    @test(
      'should render correct when button is icon type under unread status [JPT-2461]',
    )
    @mockEntity({
    ...
    @test(
      'should render correct when button is menu item type under read status',
    )
    @mockEntity({
      readStatus: READ_STATUS.READ,
    })
    t3() {
      const wrapper = mountWithTheme(
        <Read id={1234} type={BUTTON_TYPE.MENU_ITEM} />,
      );
    ...
    @test(
      'should render correct when button is menu item type under unread status',
    )
    @mockEntity({
      readStatus: READ_STATUS.UNREAD,
    })
  ...
  class read {
    @test(
      'should toast error when unread message fail for network issue [JPT-2241]',
    )
    @mockEntity({
      readStatus: READ_STATUS.READ,
    })
    ...
    async t1() {
      ...
      expect(Notification.flashToast).toHaveBeenCalledWith(
        checkNotification(
          'voicemail.prompt.notAbleToUnreadVoicemalForNetworkIssue',
        ),
      );
    }

    @test(
      'should toast error when unread message fail for server issue [JPT-2240]',
    )
    @mockEntity({
      readStatus: READ_STATUS.READ,
    })
    ...
    async t2() {
      ...
      expect(Notification.flashToast).toHaveBeenCalledWith(
        checkNotification(
          'voicemail.prompt.notAbleToUnreadVoicemalForServerIssue',
        ),
      );
    }

    @test(
      'should toast error when read message fail for network issue [JPT-2241]',
    )
    @mockEntity({
      readStatus: READ_STATUS.UNREAD,
    })
    ...
    async t3() {
      ...
      expect(Notification.flashToast).toHaveBeenCalledWith(
        checkNotification(
          'voicemail.prompt.notAbleToReadVoicemailForNetworkIssue',
        ),
      );
    }

    @test(
      'should toast error when read message fail for server issue [JPT-2240]',
    )
    @mockEntity({
      readStatus: READ_STATUS.UNREAD,
    })
    ...
    async t4() {
      ...
      expect(Notification.flashToast).toHaveBeenCalledWith(
        checkNotification(
          'voicemail.prompt.notAbleToReadVoicemailForServerIssue',
        ),
      );
    }
  }
