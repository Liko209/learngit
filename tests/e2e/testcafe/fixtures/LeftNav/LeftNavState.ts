...
// default value of leftPanelWidth
const minLeftPanelWidth = 64;
const maxLeftPanelWidth = 180;

test(formalName('Check the each user can remember the status of left navigation', ['P2', 'JPT-18']), async (t) => {
...
test(formalName('reload page keep left panel state', ['P2', 'JPT-38']), async (t) => {
  const app = new AppRoot(t);
  const loginUser = h(t).rcData.mainCompany.users[0];
