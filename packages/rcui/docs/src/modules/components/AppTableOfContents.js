...
import textToHash from "./MarkdownElement/textToHash";
...
function checkDuplication(uniq, item) {
  warning(
    !uniq[item.hash],
    `Table of content: duplicated \`${item.hash}\` item`
  );
              ...
              {this.itemsServer.map(item2 => (
                <li key={item2.text}>
                  <Link
                    block
                    color={
                      active === item2.hash ? "textPrimary" : "textSecondary"
                    }
                    href={`#${item2.hash}`}
                    ...
                    <ul className={classes.ul}>
                      {item2.children.map(item3 => (
                        <li key={item3.text}>
                          ...
                          >
                            <span
                              dangerouslySetInnerHTML={{ __html: item3.text }}
                            />
                          </Link>
                        </li>
                      ))}
