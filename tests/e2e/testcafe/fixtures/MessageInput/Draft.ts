...
test(formalName('draft', ['P0', 'JPT-139', 'Show massage draft when switching conversation']), async (t) => {
  ...
  const chain = directLogin(t).chain(async (t, h) => {
    ...
    h.log(`1.1 create A private chat conversation ${groupId1}, and show to left rail`);
    ...
    h.log(`1.2 create B private chat conversation ${groupId2}, and show to left rail`);
  });
  await chain;
  await chain.shouldNavigateTo(Send)
    .log(`2.1 select A conversation ${groupId1}`)
    .selectConversation(groupId1)
    .log('2.2 expect editor has existed')
    .expectEditorHasExisted()
    .log('2.3 typing message')
    .inputMessage(msg)
    .log(`3.1 select B conversation ${groupId2}`)
    .selectConversation(groupId2)
    .log('3.2 expect editor has existed')
    .expectEditorHasExisted()
    // todo expect groupId1 in conversation list show [draft] tag
    .log(`4.1 select A conversation ${groupId1}`)
    .selectConversation(groupId1)
