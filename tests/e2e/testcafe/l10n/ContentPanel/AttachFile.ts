...
fixture('ContentPanel/AttachFile').beforeEach(setupCase(BrandTire.RCOFFICE)).afterEach(teardownCase());

test(formalName('Check attach file button in the content panel', ['P2','Message','ContentPanel','AttachFile','V1.4','Knight.Shen']),async (t: TestController) => {
	...
	await h(t).log('Then I take screenshot', {screenshotPath: 'Jupiter_ContentPanel_AttachFileIcon'});
	...
	await h(t).log('Then I take screenshot', {screenshotPath: 'Jupiter_ContentPanel_AttachFileMenu'});
});
