...
import { getMeta, getParamsMeta } from '../../../decorators/metaUtil';
...
export class MockGlipServer {
  ...
  constructor() {
    ...
    this.applyRoute(MockGlipServer);

    this.init();
  }

  applyRoute(cls: {new(...params: any): object}) {
    const routeMetas = getMeta<IRoute<IApiContract>>(
      cls.prototype,
      META_ROUTE,
    );
    function getParamMeta(metaKey: string, prototype: any, propertyKey: string) {
      Reflect.getMetadata(metaKey, prototype, propertyKey)
    }
    routeMetas.map(({ key, meta }) => {
      const { method = 'get', path, query = {} } = meta;
      const contextParam = getParamMeta('context', cls.prototype, key);
