...
class RTCSipCallSession extends EventEmitter2 implements IRTCCallSession {
  ...
  setSipInfoIntoCallInfo(callInfo: RTCCallInfo): void {
    if (!this._session) {
      rtcLogger.warn(
        LOG_TAG,
        'Failed to set sip info into call info. Session is null'
      );
      return;
    }

    callInfo.callId = this._session.request.callId || '';
    callInfo.fromTag = this._session.request.fromTag || '';
    callInfo.toTag = this._session.request.toTag || '';

    rtcLogger.info(
      LOG_TAG,
      `set sip info callId=${callInfo.callId};fromTag=${
        callInfo.fromTag
      }; toTag=${callInfo.toTag}; to call info`
    );
    return;
  }
