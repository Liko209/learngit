...
fixture('Phone/CallHistoryAction')
  ...
  test(formalName('Check call history action tips and delete call log popup', ['P2', 'Phone', 'CallHistoryAction', 'V1.6', 'Sean.Zhuang']), async (t) => {
    ...
    await h(t).log('Then I take screenshot', { screenshotPath: 'Jupiter_Phone_CallBack' });

    await h(t).withLog('When I hover "Message" button', async () => {
      await callHistoryItem.hoverMessageButton();
    });

    await h(t).log('Then I take screenshot', { screenshotPath: 'Jupiter_Phone_Message' });

    await h(t).withLog('When I hover "Delete" button', async () => {
      await callHistoryItem.hoverDeleteButton();
    });

    await h(t).log('Then I take screenshot', { screenshotPath: 'Jupiter_Phone_Delete' });
