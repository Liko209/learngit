...
const SOURCE_CODE_ROOT_URL =
  "https://github.com/mui-org/material-ui/blob/master";
...
Any other properties supplied will be spread to the root element (${
      reactAPI.inheritance
        ? `[${
            reactAPI.inheritance.component
          }](${Router._rewriteUrlForNextExport(reactAPI.inheritance.pathname)})`
        : "native element"
    }).`;
  }
...
function generateDemos(reactAPI) {
  const pagesMarkdown = reactAPI.pagesMarkdown.reduce((accumulator, page) => {
    if (
      !DEMO_IGNORE.includes(page.filename.slice(-6)) &&
...
${pagesMarkdown
  .map(
    page =>
      `- [${pageToTitle(page)}](${_rewriteUrlForNextExport(page.pathname)})`
  )
  .join("\n")}
...
export default function generateMarkdown(reactAPI) {
  return [
    ...
    `# ${reactAPI.name} API`,
    "",
    `<p class="description">The API documentation of the ${
      reactAPI.name
    } React component. ` +
    ...
    "",
    `${generateClasses(reactAPI)}${generateInheritance(
      reactAPI
