...
class TeamSection extends BasePage {
  public shouldBeTeam() {
    return this.chain(async (t) => {
      await t.expect(team.exists).ok('Failed to find the team, probably caused by long-time loading', { timeout: 1500000 });
  ...
  public teamNameShouldChange() {
    return this.chain(async (t) => {
      await t.expect(team.exists).ok('Failed to find the team, probably caused by long-time loading', { timeout: 150000 });
      const id = (await team.getReact()).key;
