...
import { typography, palette, grey } from '../../utils/styles';
...
const StyledComponent = styled<JuiLinkProps, 'span'>('span')`
  ...
  &:active {
    color: ${({ theme, color = 'primary' }) => tinycolor(palette(color, 'main')({ theme })).setAlpha(1 - theme.palette.action.hoverOpacity * 2).toRgbString()};
  }
`;

type ILink = React.ComponentType<JuiLinkProps> & IDependencies;
