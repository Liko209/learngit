/**
 * js version, we can copy this file to execute in chrome console
 * it can help us to collect xhr/socket network data in the page.
 * expose NetworkDataTool instance in window._networkDataTool
 * startWatch/save/clear/filter
 */
/*
...
class NetworkDataTool {
    ...
    save() {
        Utils.saveBlob('network_all.json', new Blob([JSON.stringify(this._infoPool.filter(item => item.hostAlias), null, 2)]));
    }
    saveSocketOnly() {
        Utils.saveBlob('network_socket_only.json', new Blob([
            JSON.stringify(this._infoPool.filter(item => item.hostAlias && item.type === 'socket-message'), null, 2),
        ]));
    }
    saveRequestResponseOnly() {
        Utils.saveBlob('network_request_response_only.json', new Blob([
            JSON.stringify(this._infoPool.filter(item => item.hostAlias && item.type === 'request-response'), null, 2),
        ]));
    }
}
