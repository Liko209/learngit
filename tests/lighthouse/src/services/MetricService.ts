...
class MetriceService {
    ...
    async createScene(taskDto: TaskDto, scene: Scene): Promise<SceneDto> {
        ...
        return await SceneDto.create({
            taskId, name, uri, aliasUri, performance,
            accessibility, bestPractices, seo, pwa,
            startTime, endTime
        });
    }
    ...
    async createPerformance(sceneDto: SceneDto, scene: Scene) {
        ...
        let data = scene.getData();
        let { audits } = data;
        let firstContentfulPaint, firstMeaningfulPaint, speedIndex,
            firstCpuIdle, timeToInteractive, estimatedInputLatency;

        firstContentfulPaint = firstMeaningfulPaint = speedIndex = 0;
        firstCpuIdle = timeToInteractive = estimatedInputLatency = 0;
        if (audits['first-contentful-paint'] && audits['first-contentful-paint'].rawValue) {
            firstContentfulPaint = audits['first-contentful-paint'].rawValue;
        }

        if (audits['first-meaningful-paint'] && audits['first-meaningful-paint'].rawValue) {
        ...
        if (audits['estimated-input-latency'] && audits['estimated-input-latency'].rawValue) {
            estimatedInputLatency = audits['estimated-input-latency'].rawValue;
        }

        await PerformanceDto.create({
            sceneId, firstContentfulPaint, firstMeaningfulPaint, speedIndex,
            firstCpuIdle, timeToInteractive, estimatedInputLatency
        });
    }
