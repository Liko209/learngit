...
class TeamsSettingHandler extends AbstractSettingEntityHandler<
  EMAIL_NOTIFICATION_OPTIONS
> {
  ...
  async updateValue(value: EMAIL_NOTIFICATION_OPTIONS) {
    await this._profileService.updateSettingOptions([
      { value, key: SETTING_KEYS.EMAIL_TEAM },
    ]);
  }
  async fetchUserSettingEntity() {
    ...
    const settingItem: UserSettingEntity<EMAIL_NOTIFICATION_OPTIONS> = {
      ...
      value: profile[SETTING_KEYS.EMAIL_TEAM],
      id: SettingEntityIds.Notification_Teams,
  ...
  async onProfileEntityUpdate(
    ...
    if (
      profile[SETTING_KEYS.EMAIL_TEAM] !== this.userSettingEntityCache.value
    ) {
      this.notifyUserSettingEntityUpdate(await this.getUserSettingEntity());
    }
  }
}
export { TeamsSettingHandler };
