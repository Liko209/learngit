...
// TODO: Guest user
test(formalName(`Only admin and member have add members permission when add team member toggle is on`, ['P1', 'JPT-949', 'AddTeamMembersPermission', 'Potar.He']), async t => {
  const app = new AppRoot(t);
  const memberUser = h(t).rcData.mainCompany.users[4];
  const adminUser = h(t).rcData.mainCompany.users[5];
  await h(t).glip(memberUser).init();
