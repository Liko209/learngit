...
import { DirectMessagesSettingHandler } from '../DirectMessagesSettingHandler';
...
describe('DirectMessagesSettingHandler', () => {
  let profileService: ProfileService;
  let accountService: AccountService;
  let settingHandler: DirectMessagesSettingHandler;
  ...
  function setUp() {
    ...
    settingHandler = new DirectMessagesSettingHandler(profileService);
  ...
  describe('fetchUserSettingEntity()', () => {
    it('should get new messages setting value ', async () => {
      profileService.getProfile = jest.fn().mockReturnValue({
        [SETTING_KEYS.EMAIL_DM]: EMAIL_NOTIFICATION_OPTIONS.OFF,
      });
  ...
  describe('handleProfileUpdated', () => {
    it('should emit update when has subscribe update', (done: jest.DoneCallback) => {
      ...
      notificationCenter.emitEntityUpdate<Profile>(ENTITY.PROFILE, [
        {
          id: mockUserId,
          [SETTING_KEYS.EMAIL_DM]: EMAIL_NOTIFICATION_OPTIONS.EVERY_HOUR,
        } as Profile,
      ]);
      setTimeout(() => {
