...
describe('ItemAPI', () => {
  ...
  describe('requestById()', () => {
    ...
    it('glipNetworkClient.get() should be called with default path if no path found corresponding to typeId', () => {
  ...
  describe('requestRightRailItems()', () => {
    it('glipNetworkClient.get() should be called with specific path', () => {
      ...
      expect(ItemAPI.glipNetworkClient.get).toHaveBeenCalledWith('/web_client_right_rail_items', { group_id: 1 });
    });
  });
