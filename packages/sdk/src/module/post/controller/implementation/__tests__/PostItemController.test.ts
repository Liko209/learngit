...
describe('PostItemController', () => {
  ...
  describe('isValidPost', () => {
    it('should return true if post has text', () => {
      expect(postItemController.isValidPost(localPostJson4UnitTest)).toBe(true);
    });
    it('should return true if post has item_ids', () => {
      expect(
        postItemController.isValidPost({ text: '', item_ids: [1] } as Post),
      ).toBe(true);
    });
    it('should return false if post neither has text nor item_ids ', () => {
      expect(
        postItemController.isValidPost({ text: '', item_ids: [] } as Post),
      ).toBe(false);
      const result = !!postItemController.isValidPost(undefined);
      expect(result).toBe(false);
    });
  });
});
